...
contract DeployHook is Script {
    ...
    function run() external {
        ...
        require((uint160(hookAddress) & REQUIRED_FLAGS) == REQUIRED_FLAGS, "Hook address missing required flags");

        // Deploy the hook with the mined salt
        ILInsuranceHook hook = new ILInsuranceHook{salt: salt}(IPoolManager(poolManagerAddr), premiumBps, thresholdBps);
        ...
        string memory deploymentInfo = string(
            abi.encodePacked(
                ...
                "\n",
                "Premium BPS: ",
                vm.toString(premiumBps),
                "\n",
                "IL Threshold BPS: ",
                vm.toString(thresholdBps),
                "\n",
                "Salt: ",
                vm.toString(uint256(salt)),
                "\n",
                "Flags: ",
                vm.toString(uint160(address(hook)) & ((1 << 14) - 1))
            )
        );
